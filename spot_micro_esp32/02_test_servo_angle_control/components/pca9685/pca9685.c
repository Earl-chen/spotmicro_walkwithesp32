...
esp_err_t generic_write_i2c_register_two_words(uint8_t regaddr, uint16_t valueOn, uint16_t valueOff)
{
    ...
    // 发送ON值：先低字节，再高字节
    i2c_master_write_byte(cmd, valueOn & 0xff, ACK_CHECK_EN);
    i2c_master_write_byte(cmd, (valueOn >> 8) & 0xff, ACK_CHECK_EN);

    // 发送OFF值：先低字节，再高字节
    i2c_master_write_byte(cmd, valueOff & 0xff, ACK_CHECK_EN);
    i2c_master_write_byte(cmd, (valueOff >> 8) & 0xff, ACK_CHECK_EN);
...
esp_err_t setFrequencyPCA9685(uint16_t freq)
{
    ...
    // i2c_master_start(cmd);
    // i2c_master_write_byte(cmd, (PCA9685_ADDR << 1) | I2C_MASTER_READ, ACK_CHECK_EN);
    // i2c_master_read_byte(cmd, &mode1_val, NACK_VAL);
    // i2c_master_stop(cmd);
    // ret = i2c_master_cmd_begin(I2C_NUM_0, cmd, pdMS_TO_TICKS(1000));
    // i2c_cmd_link_delete(cmd);
    // if (ret != ESP_OK) return ret;
    ...
    // // 2) 进入睡眠模式（保留其他位，只设置SLEEP位）
    // uint8_t sleepmode = (oldmode & 0x7F) | 0x10; // 设置bit4为SLEEP位
    // ret = generic_write_i2c_register(MODE1, sleepmode);
    // if (ret != ESP_OK) return ret;
    ...
    // // 4) 恢复原始MODE1设置（清除SLEEP位），等待振荡器稳定
    // ret = generic_write_i2c_register(MODE1, oldmode); // 恢复原始mode1设置（SLEEP位被清除）
    // if (ret != ESP_OK) return ret;
    // vTaskDelay(pdMS_TO_TICKS(1)); // 数据手册建议延时500us，这里用1ms更安全

    // // 5) 可选：写入RESTART位来重启PWM波形（如果需要）
    // // 如果需要重启PWM输出，可以设置RESTART位：
    // // ret = generic_write_i2c_register(MODE1, oldmode | 0x80);

    // // 6) 启用AI（自动递增）位以便批量写入LED寄存器
    // uint8_t newmode = oldmode | 0x20; // AI位为bit5（值为0x20）
    // // 注意：数据手册中AI为bit5（值0x20），之前的代码使用了0xa0（0x80+0x20）
    // ret = generic_write_i2c_register(MODE1, newmode);
    // return ret;
