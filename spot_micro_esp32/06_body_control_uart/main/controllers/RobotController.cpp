...
bool RobotController::init() {
    ...
    if (hw_driver_->init(100) != Drivers::PCA9685Error::SUCCESS) {  // 修正：50Hz → 100Hz
        LOG_TAG_ERROR("ROBOT", "Failed to initialize hardware driver");
        return false;
    }

    // 创建舵机驱动
    servo_driver_ = std::make_shared<ServoDriver>(hw_driver_, config_manager_);
    if (servo_driver_->init() != ServoError::SUCCESS) {
        LOG_TAG_ERROR("ROBOT", "Failed to initialize servo driver");
        return false;
    }

    // 创建关节控制器
    joint_controller_ = std::make_shared<JointController>(servo_driver_, config_manager_);
    if (joint_controller_->init() != JointError::SUCCESS) {
        LOG_TAG_ERROR("ROBOT", "Failed to initialize joint controller");
        return false;
    }
