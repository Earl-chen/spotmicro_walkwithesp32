...
esp_err_t setFrequencyPCA9685(uint16_t freq)
{
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(tag, "Failed to set prescaler: %s", esp_err_to_name(ret));
        return ret;
    }
    ...
    ret = resetPCA9685();
    if (ret != ESP_OK) {
        ESP_LOGW(tag, "Reset may have failed, continuing: %s", esp_err_to_name(ret));
    }

    // Send to sleep again
    ret = generic_write_i2c_register(MODE1, 0x10);
    if (ret != ESP_OK) {
        ESP_LOGE(tag, "Failed to re-enter sleep mode: %s", esp_err_to_name(ret));
        return ret;
    }
    ...
    ESP_LOGI(tag, "PCA9685 frequency set to %d Hz", freq);
    return ESP_OK;
}
