...
SpotLegKinematics::SpotLegKinematics(bool is_left, float l1, float l2, float l3, float l4)
    : l1_(l1), l2_(l2), l3_(l3), l4_(l4), is_left_(is_left) {

    ESP_LOGI(TAG, "初始化%s腿运动学: L1=%.3f L2=%.3f L3=%.3f L4=%.3f",
             is_left ? "左" : "右", l1_, l2_, l3_, l4_);

    // 验证几何参数
    if (!SpotMicroGeometry::validateGeometry()) {
...
SpotLegKinematics::WorkspaceBounds SpotLegKinematics::getWorkspaceBounds() const {
    ...
    // 基于几何参数估算工作空间
    float max_reach = l3_ + l4_;
    float lateral_reach = std::sqrt(l1_*l1_ + max_reach*max_reach);

    bounds.min_x = -max_reach;
    bounds.max_x = max_reach;
    bounds.min_y = is_left_ ? l1_ : -lateral_reach;
    bounds.max_y = is_left_ ? lateral_reach : -l1_;
    bounds.min_z = -(l2_ + max_reach);
    bounds.max_z = l2_;
