...
def create_velocity_animation(gait_config, output_file, title, frames=100, dt=0.02):
    """
    创建速度控制动画
    ...
    """
    # 创建步态控制器
    gait = WalkGait(**gait_config['init'])
    
    # 应用速度设置
    if 'velocity' in gait_config:
        gait.set_velocity(**gait_config['velocity'])
...
def main():
    ...
    scenarios = [
        {
            ...
            'config': {
                ...
                'velocity': {'forward': 0.02, 'lateral': 0.0, 'yaw_rate': 0.0}
            }
        },
        {
            'name': 'forward_fast',
            ...
            'config': {
                'init': {'stride_length': 0.05, 'step_height': 0.03, 'frequency': 0.8},
                'velocity': {'forward': 0.08, 'lateral': 0.0, 'yaw_rate': 0.0}
            }
        },
        {
            'name': 'backward',
            ...
            'config': {
                'init': {'stride_length': 0.05, 'step_height': 0.03, 'frequency': 0.8},
                'velocity': {'forward': -0.05, 'lateral': 0.0, 'yaw_rate': 0.0}
            }
        },
        {
            'name': 'turn_left',
            ...
            'config': {
                'init': {'stride_length': 0.05, 'step_height': 0.03, 'frequency': 0.8},
                'velocity': {'forward': 0.05, 'lateral': 0.0, 'yaw_rate': 0.5}
            }
        },
        {
            'name': 'turn_right',
        ...
        },
        {
            ...
            'config': {
                'init': {'stride_length': 0.05, 'step_height': 0.03, 'frequency': 0.8},
                'velocity': {'forward': 0.0, 'lateral': 0.0, 'yaw_rate': 0.8}
            }
        },
    ]
    ...
    for i, scenario in enumerate(scenarios, 1):
        ...
        output_file = os.path.join(output_dir, f"velocity_{scenario['name']}.gif")
